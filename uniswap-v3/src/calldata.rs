/// ABI calldata encoding for Uniswap V3 contract calls.
...
sol! {
    ...
    // SwapRouter02 — exactInputSingle (7-field struct, NO deadline — differs from v1)
    // Selector: 0x04e45aaf
...
pub fn encode_collect(token_id: u128, recipient: &str) -> Result<String> {
...
pub fn encode_burn(token_id: u128) -> Result<String> {
    let call = burnCall {
        tokenId: U256::from(token_id),
    };
    Ok(format!("0x{}", hex::encode(call.abi_encode())))
...
/// Returns (token0, token1) sorted such that token0 < token1 (lexicographic).
/// Required by V3 pool contracts — mint reverts if token0 >= token1.
