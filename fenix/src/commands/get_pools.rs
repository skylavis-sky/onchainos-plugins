...
pub async fn run(limit: usize) -> Result<()> {
    ...
    if !http_resp.status().is_success() {
        println!(
            ...
            serde_json::json!({
                ...
                "count": 0,
                "pools": [],
                "note": "Pool list unavailable — the Goldsky subgraph is currently unreachable. Use `fenix get-quote` to check prices directly."
            })
        );
        return Ok(());
    }
    ...
    if resp.get("errors").is_some() || resp["data"]["pools"].is_null() {
        println!(
            "{}",
            serde_json::json!({
                "ok": true,
